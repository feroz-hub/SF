...
const hclCsGroupLabels: Record<string, string> = {
  client: "Clients",
...
function buildScopeGroups(allScopes: string[]): ScopeGroup[] {
  ...
  for (const scope of allScopes) {
    ...
    if (scope.toLowerCase().startsWith("hcl-cs.")) {
      const rest = scope.substring("hcl-cs.".length);
      const [resourceKey, ...permissionParts] = rest.split(".");
      const key = resourceKey || "other";
      const groupId = `hcl-cs:${key.toLowerCase()}`;
      const label = hclCsGroupLabels[key.toLowerCase()] ?? `HCL.CS: ${key}`;
    ...
    <div className="form-grid">
      <div className="form-grid" style={{ gap: "0.6rem" }}>
        ...
        <div className="form-row">
          ...
          />
          <p className="inline-message">Human-friendly name shown in HCL.CS and consent screens (max {MAX_CLIENT_NAME_LENGTH} characters).</p>
        </div>
      ...
      <div className="form-grid" style={{ gap: "0.6rem" }}>
        ...
        <div className="form-row">
          ...
          <Textarea
            ...
            placeholder="https://localhost:3000/api/auth/callback/hcl-cs"
