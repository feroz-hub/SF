...
export async function listClientActiveTokens(
  ...
  return hclCsPostWithSession<TokenModel[], ClientTokenLookupRequest>(ApiRoutes.securityToken.getByClientIds, payload);
}
...
export async function listTokensByDateRange(
  ...
  return hclCsPost<TokenModel[], DateRangeLookupRequest>(route, payload, accessToken);
}
...
export async function revokeToken(token: string, tokenTypeHint: "access_token" | "refresh_token") {
  ...
  const response = await hclCsFetch(endpoint, {
    method: "POST",
    headers: {
