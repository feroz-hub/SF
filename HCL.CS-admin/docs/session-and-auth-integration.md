# HCL.CS-Admin: Session, Login, and Logout Integration
...
│  • Else: refreshAccessToken(token) → POST HCL.CS /security/token            │
...
| Refresh token request to HCL.CS | [lib/auth.ts](lib/auth.ts) – `refreshAccessToken()` |
...
- **Correct:** Login stores HCL.CS tokens in the JWT; refresh runs when the access token is near expiry (1 min buffer); session is available server-side via `getServerSession`; all API calls use `session.accessToken`.
...
1. Call `logoutAction()` (HCL.CS SignOut + revoke tokens); errors are ignored if the session is already invalid.
...
3. NextAuth runs **CredentialsProvider** `authorize()` in [lib/auth.ts](lib/auth.ts):
   - `requestPasswordGrantTokens(userName, password)`:
     - Resolves token URL via `resolveTokenEndpoint()` (env `HCL_CS_TOKEN_ENDPOINT` or discovery `token_endpoint` or `${issuer}/security/token`).
     - POSTs to HCL.CS **token endpoint** with `grant_type=password`, `username`, `password`, client auth (Basic or client_id/client_secret in body), optional `scope`.
...
| Purpose | Endpoint (HCL.CS) | Used by |
|--------|--------------------|--------|
| Token (password grant) | `POST /security/token` (or env) | `requestPasswordGrantTokens()` |
| Token (refresh) | Same | `refreshAccessToken()` |

Discovery is optional: [lib/oidc.ts](lib/oidc.ts) `getDiscoveryDocument()` from `HCL_CS_METADATA_ADDRESS` or `{issuer}/.well-known/openid-configuration`.

### 2.3 Login integration correctness

- **Correct:** Login is fully integrated with the HCL.CS server. Credentials are validated by HCL.CS; tokens (access, refresh, id) and expiry come from the server and are stored in the JWT and session. Roles are derived from the access token and used for admin checks in [app/admin/layout.tsx](app/admin/layout.tsx).
...
- **Federated logout (end-session)**  
  [app/api/auth/federated-logout-url/route.ts](app/api/auth/federated-logout-url/route.ts) builds a HCL.CS **end_session** URL (with `client_id`, `post_logout_redirect_uri`, optional `id_token_hint`). This route is **never called** from the app. So we never redirect the user to HCL.CS’s end_session endpoint to clear server-side OIDC session.

- **Token revocation**  
  On logout we do **not** revoke the current access or refresh token via HCL.CS’s revocation endpoint. The tokens remain valid until they expire (or until revoked elsewhere, e.g. Revocation UI).
...
| **Session refresh** | Yes | Refresh token grant to HCL.CS token endpoint. |
| **Login** | Yes | Password grant to HCL.CS token endpoint; tokens and roles stored. |
...
1. **HCL.CS SignOut and token revocation**  
   [app/actions/logout.ts](app/actions/logout.ts) defines `logoutAction()`: it calls HCL.CS `signOut()` (POST `/Security/Api/Authentication/SignOut`) and then revokes the current access and refresh tokens via the revocation endpoint. Errors are swallowed so the client always proceeds to clear the session.

2. **Federated logout**  
   When `HCL_CS_ENABLE_FEDERATED_LOGOUT=true`, the Header fetches `/api/auth/federated-logout-url` (while still signed in), then after `logoutAction()` and NextAuth `signOut()`, redirects the browser to the returned end_session URL. HCL.CS clears its session and redirects back to `post_logout_redirect_uri` (e.g. `/login`).
