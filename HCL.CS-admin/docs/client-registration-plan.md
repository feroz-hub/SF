...
# Client registration: server flow, required fields, validation, and HCL.CS-admin alignment
...
```
HCL.CS-admin (Create Client)  →  POST /Security/Api/Client/RegisterClient  (body: ClientsModel)
       ↓
...
| Step | Location |
|------|----------|
| API route | [Gateway/Routes/ClientServiceRoute.cs](src/Gateway/HCL.CS.Gateway/Routes/ClientServiceRoute.cs) – `RegisterClient` |
| Route path | [ApiRoutePathConstants.RegisterClient](src/Identity/HCL.CS.Identity.Domain/Constants/ApiRoutePathConstants.cs) = `"/Security/Api/Client/RegisterClient"` |
| Service | [ClientService.RegisterClientAsync](src/Identity/HCL.CS.Identity.Application/Implementation/Api/Services/ClientService.cs) |
| Validation | [ClientModelSpecification](src/Identity/HCL.CS.Identity.Application/Implementation/Api/Specifications/ClientModelSpecification.cs) (CrudMode.Add) |
| Model | [ClientsModel](src/Identity/HCL.CS.Identity.Domain/Models/Endpoint/ClientsModel.cs) |
...
All rules below run for **Add** (create). Rules are in [ClientModelSpecification.cs](src/Identity/HCL.CS.Identity.Application/Implementation/Api/Specifications/ClientModelSpecification.cs).
...
From [TokenConfig.cs](src/Identity/HCL.CS.Identity.Domain/Configurations/Endpoint/TokenConfig.cs) (defaults; can be overridden by config):
...
## 3. HCL.CS-admin: create-client payload and validation

### 3.1 Where it happens

- **Form and submit:** [components/modules/clients/ClientsModule.tsx](HCL.CS-admin/components/modules/clients/ClientsModule.tsx) – Create dialog, `onSubmitCreate` → `createClientAction(toPayload(form))`.
- **Action and model:** [app/admin/clients/actions.ts](HCL.CS-admin/app/admin/clients/actions.ts) – `formSchema.safeParse(input)`, then `buildCreateModel(parsed.data, actor)`.
- **API:** [lib/api/clients.ts](HCL.CS-admin/lib/api/clients.ts) – `createClient(model)` → POST `ApiRoutes.client.registerClient` with body `ClientsModel`.
...
- **CreatedBy:** actor (name or email or `"hcl-cs-admin"`)  
...
## 5. Recommended checks in HCL.CS-admin (implemented)
