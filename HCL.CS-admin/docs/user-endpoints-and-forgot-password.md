...
## 2. Forgot Password flow in HCL.CS-admin
...
3. **Email**  
   - User receives email from HCL.CS (SMTP) with the reset token (or link containing it), per server notification config.
...
- **Anonymous calls:** `lib/api/client.ts` exposes `hclCsPostAnonymous(path, payload)` (no `Authorization` header). Used by `lib/api/passwordReset.ts` for `generatePasswordResetTokenAnonymous` and `resetPasswordAnonymous`.
