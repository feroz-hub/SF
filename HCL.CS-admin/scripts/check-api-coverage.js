...
function relativeToHclCsAdmin(...segments) {
...
function main() {
  const repoRootFromHclCsAdmin = relativeToHclCsAdmin("..");

  const openApiPath = path.resolve(repoRootFromHclCsAdmin, "docs", "api", "all-apis.openapi.json");
  const mdPath = path.resolve(repoRootFromHclCsAdmin, "docs", "api", "all-apis.md");
  const routesPath = relativeToHclCsAdmin("lib", "api", "routes.ts");
  ...
  printList("[api-coverage] MISSING in HCL.CS-admin route map", missing);
  printList("[api-coverage] EXTRA in HCL.CS-admin route map (not in docs)", extra);
