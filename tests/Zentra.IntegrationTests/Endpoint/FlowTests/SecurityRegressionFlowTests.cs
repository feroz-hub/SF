...
public class SecurityRegressionFlowTests : ZentraFakeSetup
{
    private const string Category = "SecurityRegression";
    private const string RedirectUri = "http://127.0.0.1:63562/";
    private const string StrictPkceClient = "Zentra S256 Client";
    ...
    public async Task PkcePlainDowngradeAttempt_MustBeRejected()
    {
        ...
        var requestUrl = CreateAuthorizeRequestUrl(
            client.ClientId,
            "code",
            "openid email profile offline_access zentra.client",
            responseMode: "query",
            prompt: "none",
    ...
    {
        var client = await FetchClientDetails("Zentra HS256");
        client.Should().NotBeNull();
    ...
    public async Task AudienceMismatch_MustFailTokenValidation()
    {
        ...
        var rawToken = await validator.ValidateToken(token.access_token, "security.zentra.com", "wrong.audience");
    ...
    private async Task<(ClientsModel Client, string Code, string CodeVerifier)> GetAuthorizationCodeAsync()
    {
        ...
        var authorizeRequest = CreateAuthorizeRequestUrl(
            client.ClientId,
            "code",
            "openid email profile phone offline_access zentra.client",
            responseMode: "query",
            prompt: "none",
