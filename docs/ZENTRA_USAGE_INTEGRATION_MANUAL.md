...
1. [Introduction to Zentra](#1-introduction-to-zentra)
2. [Core Features Overview](#2-core-features-overview)
3. [Architecture Overview](#3-architecture-overview)
4. [How to Setup Zentra](#4-how-to-setup-zentra-step-by-step)
...
## 1. Introduction to Zentra

### 1.1 What is Zentra?

Zentra is an OAuth 2.0 and OpenID Connect (OIDC) authorization server. It authenticates users and issues security tokens that applications use to access protected resources.

Think of Zentra as a security checkpoint:
- Users prove their identity to Zentra
- Zentra issues a token (like a temporary badge)
- Applications present this token to access APIs
- APIs validate the token with Zentra before allowing access

### 1.2 What Problem Does Zentra Solve?

**Without Zentra (The Problem):**
```
┌─────────┐     ┌─────────┐     ┌─────────┐
│  App A  │     │  App B  │     │  App C  │
│  ─────  │     │  ─────  │     │  ─────  │
...
**What it does:**
The most secure OAuth flow for web applications. User authenticates with Zentra, Zentra redirects back with a code, app exchanges code for tokens.
...
2. App redirects to Zentra with PKCE parameters
3. User enters credentials on Zentra
4. Zentra redirects back with authorization code
...
2. Service A authenticates with Zentra using client ID + secret
3. Zentra returns access token
...
**What JWT does:**
JSON Web Tokens contain claims (user info, permissions) signed by Zentra. APIs can validate them without calling Zentra.
...
│  ─────────────────────                                                      │
│                                                                             │
│  User ──► Browser ──► /authorize ──► Zentra ──► Login Page                 │
│                               (with PKCE)                                   │
│                                                                             │
│  User enters credentials                                                    │
│                                                                             │
│  Zentra ──► Redirect ──► Browser ──► /callback ──► Client App              │
│                    (with authorization code)                                │
│                                                                             │
│  STEP 2: TOKEN EXCHANGE                                                     │
│  ──────────────────────                                                     │
│                                                                             │
│  Client App ──► POST /token ──► Zentra                                     │
│              (code + code_verifier + client_secret)                         │
│                                                                             │
│  Zentra ──► Response ──► Client App                                        │
│              {                                                              │
...
## 4. How to Setup Zentra (Step-by-Step)
...
```bash
# Required
export ZENTRA_DB_CONNECTION_STRING="Server=localhost;Database=ZentraDev;User Id=sa;Password=YourPassword;"
export ZENTRA_TOKEN_ISSUER_URI="https://localhost:5001"

# Optional (self-signed certs will be auto-generated if not provided)
export ZENTRA_RSA_SIGNING_CERT_BASE64=""
export ZENTRA_ECDSA_SIGNING_CERT_BASE64=""
export ZENTRA_SIGNING_CERT_PASSWORD=""
```
...
```bash
cd installer/Zentra.Installer.Mvc
dotnet run
...
```bash
# For SQL Server
dotnet ef database update --project src/Identity/Zentra.Identity.Persistence

# Or use provided SQL scripts
cd scripts/seed/Sql
sqlcmd -S localhost -d ZentraDev -i ZentraSqlV1.sql
```
...
|----------|-------------|---------|
| `ZENTRA_DB_CONNECTION_STRING` | Database connection | `Server=prod-db;Database=Zentra;...` |
| `ZENTRA_TOKEN_ISSUER_URI` | Public URL of Zentra | `https://auth.yourcompany.com` |
| `ZENTRA_RSA_SIGNING_CERT_BASE64` | RSA certificate (Base64) | `MIIFXTCCA0WgAwIBAgIQC...` |
| `ZENTRA_ECDSA_SIGNING_CERT_BASE64` | ECDSA certificate (Base64) | `MIIBkTCB+wIJAKHBfpE...` |
| `ZENTRA_SIGNING_CERT_PASSWORD` | Certificate password | `SecurePassword123!` |
| `ZENTRA_RSA_SIGNING_KID` | RSA key identifier | `zentra-rsa-2026-q1` |
| `ZENTRA_ECDSA_SIGNING_KID` | ECDSA key identifier | `zentra-ecdsa-2026-q1` |
...
3. Update `ZENTRA_*_SIGNING_KID` to new key
...
Edit `demos/Zentra.Demo.Server/Program.cs`:
...
│  • Stored in database: Zentra_SecurityTokens                               │
...
• Cannot forge tokens
• Zentra private key stays secure
```
...
public async Task<IActionResult> Logout()
{
    ...
    // 3. Redirect to Zentra logout (ends SSO session)
    return SignOut(
...
const logout = async () => {
  ...
  // 2. Sign out (redirects to Zentra logout page)
...
export ZENTRA_RSA_SIGNING_CERT_BASE64=$(base64 new-rsa-cert.pem)
export ZENTRA_RSA_SIGNING_KID="zentra-rsa-2026-q2"
```
...
grep "TokenReuseDetected" /var/log/zentra/app.log

# Find failed auth by IP
grep "Authentication failed" /var/log/zentra/app.log | \
  awk '{print $5}' | sort | uniq -c | sort -rn | head -10

# Find rate limit hits
grep "Rate limit exceeded" /var/log/zentra/app.log
```
...
**Configuration:**
- Zentra connects to Active Directory
...
{
    ValidAudience = "yourcompany.api", // Must match Zentra config
    // ...
};
```
...
# Query Zentra database:
SELECT * FROM Zentra_SecurityTokens 
...
### Does Zentra support multi-tenancy?

**Answer:** Not natively. Each tenant requires a separate Zentra instance or you can implement tenant isolation using:
...
```bash
export ZENTRA_RSA_SIGNING_CERT_BASE64=$(base64 your-cert.pfx)
export ZENTRA_SIGNING_CERT_PASSWORD="your-password"
```
...
**Answer:** For many use cases, yes. Consider Zentra if:
...
2. Linking external accounts to Zentra users
3. Using Zentra for session management
...
- Modify views in `demos/Zentra.Demo.Server/Views/`
- Add CSS/JS to `wwwroot/`
- Or build your own login UI using Zentra APIs

### How do I backup Zentra?
...
- Building custom admin interface using Zentra APIs
...
- [ ] `ZENTRA_SIGNING_CERT_PASSWORD` is strong (>32 chars, random)
...
**Version:** 1.0  
**Last Updated:** February 2026  
**Maintained By:** Zentra Documentation Team  
**Feedback:** Submit issues to documentation repository

---

*This manual provides practical guidance for integrating with and deploying Zentra. For additional support, refer to the API reference documentation or contact the development team.*
