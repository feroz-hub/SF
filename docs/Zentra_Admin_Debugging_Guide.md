# Zentra Admin - Debugging & Development Guide
...
| **Project Path** | `Zentra-admin/` |
| **Framework** | Next.js 14.2.5 |
| **React Version** | 18.3.1 |
| **Authentication** | NextAuth.js 4.24.7 |
| **Language** | TypeScript 5.5.4 |
| **Purpose** | Administrative UI for Zentra Identity Server |
...
│  │  ┌─────────────┐  ┌─────────────┐  ┌─────────────┐  ┌────────────┐ │   │
│  │  │ Credentials │──│   JWT       │──│  Session    │──│  Zentra    │ │   │
...
**File:** `Zentra-admin/app/layout.tsx`
...
**File:** `Zentra-admin/app/api/auth/[...nextauth]/route.ts`
...
- `/api/auth/callback/zentra` - OAuth callback
...
| `ZENTRA_ISSUER` | Yes | Zentra server base URL | `https://localhost:5001` |
| `ZENTRA_CLIENT_ID` | Yes | OAuth client ID | `zentra-admin-client` |
| `ZENTRA_CLIENT_SECRET` | Yes | OAuth client secret | `client-secret` |
| `ZENTRA_API_BASE_URL` | No | Zentra API base (defaults to issuer) | `https://localhost:5001` |
| `ZENTRA_SCOPES` | No | Requested OAuth scopes | See default below |
| `ZENTRA_ENABLE_FEDERATED_LOGOUT` | No | Use IdP logout | `false` |
| `ZENTRA_ALLOW_INSECURE_TLS` | No | Dev only: skip TLS verify | `false` |

### Default Scopes

```
openid profile email offline_access phone 
zentra.apiresource zentra.client zentra.user zentra.role 
zentra.identityresource zentra.adminuser zentra.securitytoken
```
...
ZENTRA_ISSUER=https://localhost:5001
ZENTRA_CLIENT_ID=your-client-id
ZENTRA_CLIENT_SECRET=your-client-secret
```
...
export const authOptions: NextAuthOptions = {
  ...
  providers: [
    CredentialsProvider({
      name: "Zentra Credentials",
      credentials: {
        username: { label: "Username", type: "text" },
        password: { label: "Password", type: "password" }
      },
      authorize: async (credentials) => {
        // Password grant to Zentra server
...
Name: Zentra Admin - Debug
URL: https://localhost:3000
Remote URLs:
    http://localhost:3000 → file:///path/to/Zentra-admin
```
...
```json
{
  ...
  "configurations": [
    ...
    },
    {
      ...
      "request": "launch",
      "url": "https://localhost:3000",
      "webRoot": "${workspaceFolder}/Zentra-admin"
    },
    {
...
**Common Issues:**
- Wrong ZENTRA_ISSUER URL
- Client not registered in Zentra
...
| `/api/auth/callback/zentra` | GET/POST | OAuth callback |
...
### Zentra API Endpoints (called from lib/api/*)
...
# Run without HTTPS (not recommended for Zentra)
npm run dev
...
# Add to .env.local:
ZENTRA_ALLOW_INSECURE_TLS=true
```
...
4. Zentra server accepts the token issuer
```
...
```bash
# Check:
1. User has admin role in Zentra
...
3. `GET /Security/Api/*` - API calls to Zentra
