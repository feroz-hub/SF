...
# PreferredAudience column (HclCs_Clients)
...
If you use the **HCL.CS Installer** (HCL.CS.Installer.Mvc):
...
2. Run migrations. The installer will:
   ...
   - Run post-migration steps, including **EnsurePreferredAudienceColumnAsync**, which adds `PreferredAudience` to `HclCs_Clients` if it is missing (idempotent per provider).
...
```sql
IF COL_LENGTH('HclCs_Clients', 'PreferredAudience') IS NULL
...
2. From the solution directory, set the startup project to the API (or the project that references Persistence and has the connection string):
   ...
   dotnet ef migrations add AddClientPreferredAudience --project ../HCL.CS.Identity.Persistence
   ```
3. Apply the migration:
   ```bash
   dotnet ef database update --project ../HCL.CS.Identity.Persistence
   ```

Note: The Identity API uses `ApplicationDbContext` from HCL.CS.Identity.Persistence. The **installer** uses its own DbContext and migration history; adding a migration in the Persistence project creates a separate migration history. For a single production database, prefer either the installer path (Option 1) or manual SQL (Option 2) so you don’t mix two migration histories.
