# Zentra.Demo.Server - Debugging & Development Guide
...
| **Project Path** | `demos/Zentra.Demo.Server/` |
| **Project File** | `Zentra.DemoServerApp.csproj` |
| **Framework** | .NET 8.0 |
...
```
┌─────────────────────────────────────────────────────────────────────────────┐
│                         Zentra.Demo.Server (.NET 8)                         │
│                         OAuth2/OIDC Identity Server                         │
├─────────────────────────────────────────────────────────────────────────────┤
│  ┌──────────────┐  ┌──────────────┐  ┌──────────────┐  ┌─────────────────┐ │
│  │   Program.cs │  │   Controllers│  │   Zentra     │  │   Certificate   │ │
│  │   (Entry)    │──│   (MVC)      │──│   Services   │──│   Management    │ │
│  └──────────────┘  └──────────────┘  └──────────────┘  └─────────────────┘ │
│         │                 │                  │                              │
│         ▼                 ▼                  ▼                              │
│  ┌─────────────────────────────────────────────────────────────────────┐   │
│  │                      Zentra.Hosting (NuGet)                         │   │
│  │  ┌─────────────┐ ┌─────────────┐ ┌─────────────┐ ┌─────────────┐    │   │
...
**File:** `demos/Zentra.Demo.Server/Program.cs`
...
// Line 26-28 - Zentra Services Registration
...
Program.cs
    │
    ├─> LoadZentraConfiguration()          [Line 178-220]
    ...
    │
    ├─> AddZentra()                        [ZentraExtension.cs]
    │   └─> Configure all Zentra services
    │
...
1. Program.cs Execution
   ...
   │   ├── ZENTRA_RSA_SIGNING_CERT_BASE64 / PATH
   │   └── ZENTRA_ECDSA_SIGNING_CERT_BASE64 / PATH
   │
   ...
   │   ├── AddZentra()               → Core identity services
   ...
   │   ├── UseHttpsRedirection()
   │   ├── UseStaticFiles()
   │   ├── UseZentraCorrelationId()
   │   ├── UseZentraRequestObservability()
   │   ├── UseSession()
   │   ├── UseRouting()
   │   ├── UseCors()
   │   ├── UseRateLimiter()
   ...
   │   ├── UseAuthentication()
   │   ├── UseAuthorization()
   │   ├── UseZentraEndpoint()         → OAuth/OIDC endpoints
   │   ├── UseZentraApi()              → Management API
...
```
demos/Zentra.Demo.Server/
│
├── Program.cs                          # Application entry point
├── Zentra.DemoServerApp.csproj         # Project file
│
├── Controllers/
...
|----------|---------|---------|
| `ZENTRA_DB_CONNECTION_STRING` | Database connection | `Host=localhost;...` |
| `ZENTRA_SIGNING_CERT_PASSWORD` | Certificate password | `YourSecurePassword` |

### Certificate Variables (choose one per cert)

| Variable | Purpose |
|----------|---------|
| `ZENTRA_RSA_SIGNING_CERT_BASE64` | RSA cert as base64 |
| `ZENTRA_RSA_SIGNING_CERT_PATH` | Path to RSA cert file |
| `ZENTRA_RSA_SIGNING_KID` | RSA key ID |
| `ZENTRA_ECDSA_SIGNING_CERT_BASE64` | ECDSA cert as base64 |
| `ZENTRA_ECDSA_SIGNING_CERT_PATH` | Path to ECDSA cert file |
| `ZENTRA_ECDSA_SIGNING_KID` | ECDSA key ID |
...
Name: Zentra.Demo.Server
Project: Zentra.DemoServerApp
Working Directory: /path/to/demos/Zentra.Demo.Server
Environment Variables:
    ZENTRA_DB_CONNECTION_STRING=Host=localhost;...
    ZENTRA_SIGNING_CERT_PASSWORD=yourpassword
```
...
// Type: Zentra.Domain.ZentraException
```
  ...
  "profiles": {
    "Zentra.Demo.Server": {
      ...
      "environmentVariables": {
        ...
        "ZENTRA_DB_CONNECTION_STRING": "Host=localhost;Database=zentra;Username=postgres;Password=postgres",
        "ZENTRA_SIGNING_CERT_PASSWORD": "YourPassword"
      },
...
### Zentra Extension Services

**File:** `src/Identity/Zentra.Identity.API/Extensions/ZentraExtension.cs`
...
### ZentraBuilder Middleware

**File:** `src/Identity/Zentra.Identity.API/Extensions/ZentraBuilder.cs`

```csharp
// Extension Methods
UseZentraSecurityHeaders()      // Security headers
UseZentraCorrelationId()        // X-Correlation-ID
UseZentraRequestObservability() // Request logging
UseZentraEndpoint()             // OAuth/OIDC endpoints
UseZentraApi()                  // Management API
MapZentraHealthChecks()         // Health check endpoints
```
...
// Verify:
- ZENTRA_RSA_SIGNING_CERT_BASE64 or PATH is set
- ZENTRA_ECDSA_SIGNING_CERT_BASE64 or PATH is set  
- ZENTRA_SIGNING_CERT_PASSWORD is correct
...
|----------|--------|-------------|------------|
| `/.well-known/openid-configuration` | GET | Discovery document | ZentraEndpoint |
| `/.well-known/jwks` | GET | JSON Web Key Set | ZentraEndpoint |
| `/security/authorize` | GET | Authorization endpoint | ZentraEndpoint |
| `/security/token` | POST | Token endpoint | ZentraEndpoint |
| `/security/introspect` | POST | Token introspection | ZentraEndpoint |
| `/security/revocation` | POST | Token revocation | ZentraEndpoint |
| `/security/endsession` | GET/POST | End session (logout) | ZentraEndpoint |
| `/security/userinfo` | GET | User info endpoint | ZentraEndpoint |
...
| Endpoint | Purpose |
|----------|---------|
| `/api/*` | Management API (via ZentraApiMiddleware) |
...
export ZENTRA_DB_CONNECTION_STRING="Host=localhost;Database=zentra;Username=postgres;Password=postgres"
export ZENTRA_SIGNING_CERT_PASSWORD="YourPassword"

# Set environment variables (Windows PowerShell)
$env:ZENTRA_DB_CONNECTION_STRING="Host=localhost;Database=zentra;Username=postgres;Password=postgres"
$env:ZENTRA_SIGNING_CERT_PASSWORD="YourPassword"
