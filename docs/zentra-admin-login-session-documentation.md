# Zentra-Admin: Full Analysis — Login, Session, and Session Expiry

This document describes the Zentra-Admin application: its structure, how login works (including with the Demo Server), how the session is maintained, and how session expiry is handled, with concrete implementation references.

---

## 1. Zentra-Admin Overview

### 1.1 What It Is

**Zentra-Admin** is a Next.js administrative console for the Zentra identity platform. It allows administrators to manage users, roles, clients, API resources, identity resources, security tokens, and audit logs. It authenticates against Zentra's token endpoint (typically served by **Zentra.Demo.Server** or the Identity API) and uses the returned tokens for all API calls.

### 1.2 Tech Stack and Structure

| Layer | Technology | Location |
|-------|------------|----------|
| Framework | Next.js (App Router) | `Zentra-admin/` |
...
| Backend API | Zentra Identity API / Demo Server | `ZENTRA_ISSUER`, `ZENTRA_API_BASE_URL`, `ZENTRA_DEMO_SERVER_BASE_URL` |
...
Authentication and session behaviour depend on these variables (see `Zentra-admin/.env.example`):
...
| `ZENTRA_ISSUER` | Zentra identity issuer base URL (e.g. `https://localhost:5001`). |
| `ZENTRA_CLIENT_ID` / `ZENTRA_CLIENT_SECRET` | OAuth client credentials for the admin client in Zentra. **Required.** |
| `ZENTRA_API_BASE_URL` | Base URL for `/Security/Api/*` calls (defaults to issuer). |
| `ZENTRA_DEMO_SERVER_BASE_URL` | Demo Server base URL for health check and Google sign-in redirect. Defaults to issuer. |
| `ZENTRA_SCOPES` | Space-separated scopes requested at login. |
| `ZENTRA_TOKEN_ENDPOINT` / `ZENTRA_REVOCATION_ENDPOINT` | Optional; otherwise from OIDC discovery. |
| `ZENTRA_POST_LOGOUT_REDIRECT_URI` | Where to redirect after federated end-session. |
| `ZENTRA_ENABLE_FEDERATED_LOGOUT` | If `true`, logout redirects to Zentra end_session. |
...
**File:** `Zentra-admin/app/(auth)/login/page.tsx`
...
4. **`requestPasswordGrantTokens(userName, password)`** in `lib/auth.ts`:
   - Resolves token endpoint via `resolveTokenEndpoint()` (`lib/oidc.ts`): `ZENTRA_TOKEN_ENDPOINT` or discovery `token_endpoint` or `${issuer}/security/token`.
...
3. **CredentialsProvider** `authorize()` receives `credentials.code`. In `lib/auth.ts`:
   ...
   - Zentra's token endpoint must support the `user_code` grant (e.g. via `UserCodeFlowSpecification` and `TokenRequestValidator` handling `OpenIdConstants.GrantTypes.UserCode`).
...
| Field | Description |
|-------|-------------|
| `accessToken` | Zentra access token (Bearer) for API calls. |
...
- Storing Zentra tokens in the JWT cookie.
...
| **API calls** | `requireAccessToken()` in `lib/api/client.ts`: calls `auth()` and returns `session.accessToken`; used by `zentraPostWithSession`, `zentraGetWithSession` and all server-side API modules. |
...
- `resolveTokenEndpoint()`: Uses `ZENTRA_TOKEN_ENDPOINT` if set; otherwise OIDC discovery from `ZENTRA_METADATA_ADDRESS` or `{issuer}/.well-known/openid-configuration`; fallback `{issuer}/security/token`.
...
- **`lib/api/client.ts`:**
  - **`requireAccessToken()`** throws **`ZentraApiError`** with status `401` if there is no session or no `session.accessToken`.
  - All Zentra API calls (e.g. `zentraPost`, `zentraGet`) that get a 401 from the server throw `ZentraApiError` with `statusCode: 401`.
- **`apiErrorMessage(error)`:** For `ZentraApiError` with status 401, returns **"Session expired. Please sign in again."**
- **`getLoadErrorInfo(error)`:** Returns `{ message, isUnauthorized }`; `isUnauthorized` is true when the error is a 401 `ZentraApiError` (via **`isUnauthorizedError()`**).
...
1. Calls **`logoutAction()`** (server action): Zentra SignOut API and revoke access + refresh tokens. Errors are ignored so the client always proceeds.
...
- SignInAgainButton forcing a full logout (Zentra + NextAuth) and a full navigation to `/login`.

### 4.6 Logout and Revocation

- **`app/actions/logout.ts` — `logoutAction()`:**
  - Gets session via `auth()`.
  - If session has tokens: calls Zentra SignOut API (`signOut()`), then revokes access token and, if present, refresh token via **`lib/api/revocation.ts`** (`revokeToken`).
...
So both "Sign in again" (expired session) and normal "Logout" ensure the Zentra session and tokens are revoked when possible, and the NextAuth cookie is always cleared.
...
| **Session expiry (401)** | `lib/api/client.ts` — 401 → `ZentraApiError`, `apiErrorMessage()`, `getLoadErrorInfo()`, `isUnauthorizedError()`. |
...
- **Session and auth integration (including logout):** `Zentra-admin/docs/session-and-auth-integration.md`
- **Google login plan:** `docs/auth/google-login-zentra-admin-plan.md`
- **Demo Server debugging:** `docs/Zentra.Demo.Server_Debugging_Guide.md`

This document and the above together give a full picture of Zentra-Admin authentication, Demo Server login (including Google), session maintenance, and session expiry handling with implementation pointers.
