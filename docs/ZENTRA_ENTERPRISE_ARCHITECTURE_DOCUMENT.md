# ZENTRA — Enterprise Architecture & Technical Strategy Document
...
**Zentra's Architecture Solution:**
...
│                          │                                            │
│                    ┌───────────┐                                      │
│                    │  ZENTRA   │  ← Single Identity Provider          │
│                    │   IAM     │     (OAuth 2.0 + OIDC)               │
│                    └───────────┘                                      │
└──────────────────────────────────────────────────────────────────────┘
```
...
| Standard | Purpose | Zentra Implementation |
|----------|---------|----------------------|
...
**Zentra OAuth 2.0 Grant Types Supported:**
...
```
┌─────────────────────────────────────────────────────────────────────┐
│                MULTI-TENANT ARCHITECTURE WITH ZENTRA                │
├─────────────────────────────────────────────────────────────────────┤
│                                                                     │
│   ┌─────────────────────────────────────────────────────────────┐  │
│   │                      ZENTRA IAM                              │  │
...
| Level | Description | Zentra Status |
|-------|-------------|---------------|
...
```
┌──────────┐     ┌──────────┐     ┌──────────┐     ┌──────────┐
│  User    │────>│  App A   │────>│  ZENTRA  │────>│  App B   │
│(Employee)│     │(Portal)  │     │  (IdP)   │     │(HR System)│
└──────────┘     └──────────┘     └────┬─────┘     └──────────┘
                                       │
                                       │ 1. User logs in once
                                       │ 2. Zentra issues SSO session
...
**Zentra Fit:** Authorization Code Flow with persistent sessions.
...
├─────────────────────────────────────────────────────────────────┤
│                                                                 │
│  Client     API Gateway       Zentra         Microservices     │
│    │            │              │                  │             │
│    │ ─────────>│              │                  │             │
│    │  Request  │              │                  │             │
│    │           │ ────────────>│                  │             │
...
**Zentra Fit:** JWT validation via JWKS endpoint, scope-based authorization.
...
**Zentra Fit:** Custom claims for tenant identification, RBAC per tenant.
...
**Zentra Fit:** Native PKCE support with `RequirePkce` client configuration.
...
### Zentra as Centralized Identity Provider

```
┌─────────────────────────────────────────────────────────────────────────┐
│                    RENTFLOW + ZENTRA INTEGRATION                        │
├─────────────────────────────────────────────────────────────────────────┤
│                                                                         │
│   ┌─────────────────────────────────────────────────────────────────┐  │
│   │                          ZENTRA IAM                              │  │
...
```csharp
// Zentra Token Claims for RENTFLOW
{
  "sub": "user-uuid",
...
├─────────────────────────────────────────────────────────────────┤
│                                                                 │
│  Tenant Mobile App              Zentra Server                  │
│  ─────────────────              ─────────────                  │
│                                                                 │
│  ┌─────────────────┐                                            │
│  │ 1. User opens   │                                            │
...
├─────────────────────────────────────────────────────────────────┤
│                                                                 │
│  Zentra provides foundation for AI-powered features:            │
│                                                                 │
...
| Aspect | Current | With Zentra | Scaling Strategy |
|--------|---------|-------------|------------------|
...
dotnet build Zentra.sln --configuration Release
...
# Edit: Installer/Zentra.DBMigration/appsettings.json

{
  "ConnectionStrings": {
    "DefaultConnection": "Server=localhost;Database=ZentraAuth;User Id=sa;Password=YourPassword;TrustServerCertificate=True;"
  }
}

# 2. Run database migrations
cd Installer/Zentra.DBMigration
dotnet run
...
cd Zentra-Demo/Zentra.DemoServerApp/Zentra.DemoServerApp
...
cd Zentra-Demo/Zentra.DemoClientCoreMvcApp/Zentra.DemoClientCoreMvcApp
...
# - Click Login
# - Redirected to Zentra auth server
# - Enter credentials
# - Consent to scopes
...
public class TokenValidationParameters
{
    ...
    ValidIssuer = "https://auth.zentra.com";
...
| OWASP Top 10 | Zentra Mitigation |
|--------------|-------------------|
...
services.AddOpenTelemetry()
    .WithTracing(builder =>
    {
        builder
            ...
            .AddSource("Zentra.Authentication")
...
Zentra/
├── Zentra/
│   ├── Zentra.Domain/                    # Entities, Models, Enums
│   ├── Zentra.DomainServices/            # Repository & Service Interfaces
│   ├── Zentra.DomainValidation/          # Validation Logic
│   ├── Zentra.Infrastructure.Data/       # EF Core, Repositories
│   ├── Zentra.Infrastructure.Services/   # Email, SMS, Logging
│   ├── Zentra.Infrastructure.Resources/  # Localization
│   ├── Zentra.Service/                   # Business Logic Implementation
│   ├── Zentra.Service.Interfaces/        # Service Contracts
│   ├── Zentra.ProxyService/              # API Gateway
│   └── Zentra.Hosting/                   # DI Extensions
│
├── Zentra-Demo/
│   ├── Zentra.DemoServerApp/          # OAuth Server Demo
│   ├── Zentra.DemoClientCoreMvcApp/   # MVC Client Demo
│   ├── Zentra.DemoClientWpfApp/       # WPF Client Demo
│
├── Installer/
│   ├── Zentra.DBMigration/               # Database Migration Tool
...
|-------|---------|----------|
| `ZentraConfig` | Root configuration | `Zentra.Domain` |
| `SystemSettings` | Application settings | `Zentra.Domain` |
| `TokenSettings` | OAuth/OIDC configuration | `Zentra.Domain` |
| `UserConfig` | User policy settings | `Zentra.Domain.Configurations` |
| `PasswordConfig` | Password policy | `Zentra.Domain.Configurations` |
| `TokenExpiration` | Token lifetime rules | `Zentra.Domain.Configurations` |
...
*This document provides a comprehensive analysis of the Zentra authentication framework for enterprise architecture review. For implementation questions or clarifications, refer to the in-code documentation and demo applications.*
