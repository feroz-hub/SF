# HCL.CS-Admin: Full Analysis — Login, Session, and Session Expiry

This document describes the HCL.CS-Admin application: its structure, how login works (including with the Demo Server), how the session is maintained, and how session expiry is handled, with concrete implementation references.

---

## 1. HCL.CS-Admin Overview

### 1.1 What It Is

**HCL.CS-Admin** is a Next.js administrative console for the HCL.CS identity platform. It allows administrators to manage users, roles, clients, API resources, identity resources, security tokens, and audit logs. It authenticates against HCL.CS's token endpoint (typically served by **HCL.CS.Demo.Server** or the Identity API) and uses the returned tokens for all API calls.

### 1.2 Tech Stack and Structure

| Layer | Technology | Location |
|-------|------------|----------|
| Framework | Next.js (App Router) | `HCL.CS-admin/` |
...
| Backend API | HCL.CS Identity API / Demo Server | `HCL_CS_ISSUER`, `HCL_CS_API_BASE_URL`, `HCL_CS_DEMO_SERVER_BASE_URL` |
...
Authentication and session behaviour depend on these variables (see `HCL.CS-admin/.env.example`):
...
| `HCL_CS_ISSUER` | HCL.CS identity issuer base URL (e.g. `https://localhost:5001`). |
| `HCL_CS_CLIENT_ID` / `HCL_CS_CLIENT_SECRET` | OAuth client credentials for the admin client in HCL.CS. **Required.** |
| `HCL_CS_API_BASE_URL` | Base URL for `/Security/Api/*` calls (defaults to issuer). |
| `HCL_CS_DEMO_SERVER_BASE_URL` | Demo Server base URL for health check and Google sign-in redirect. Defaults to issuer. |
| `HCL_CS_SCOPES` | Space-separated scopes requested at login. |
| `HCL_CS_TOKEN_ENDPOINT` / `HCL_CS_REVOCATION_ENDPOINT` | Optional; otherwise from OIDC discovery. |
| `HCL_CS_POST_LOGOUT_REDIRECT_URI` | Where to redirect after federated end-session. |
| `HCL_CS_ENABLE_FEDERATED_LOGOUT` | If `true`, logout redirects to HCL.CS end_session. |
...
**File:** `HCL.CS-admin/app/(auth)/login/page.tsx`
...
4. **`requestPasswordGrantTokens(userName, password)`** in `lib/auth.ts`:
   - Resolves token endpoint via `resolveTokenEndpoint()` (`lib/oidc.ts`): `HCL_CS_TOKEN_ENDPOINT` or discovery `token_endpoint` or `${issuer}/security/token`.
...
3. **CredentialsProvider** `authorize()` receives `credentials.code`. In `lib/auth.ts`:
   ...
   - HCL.CS's token endpoint must support the `user_code` grant (e.g. via `UserCodeFlowSpecification` and `TokenRequestValidator` handling `OpenIdConstants.GrantTypes.UserCode`).
...
| Field | Description |
|-------|-------------|
| `accessToken` | HCL.CS access token (Bearer) for API calls. |
...
- Storing HCL.CS tokens in the JWT cookie.
...
| **API calls** | `requireAccessToken()` in `lib/api/client.ts`: calls `auth()` and returns `session.accessToken`; used by `hclCsPostWithSession`, `hclCsGetWithSession` and all server-side API modules. |
...
- `resolveTokenEndpoint()`: Uses `HCL_CS_TOKEN_ENDPOINT` if set; otherwise OIDC discovery from `HCL_CS_METADATA_ADDRESS` or `{issuer}/.well-known/openid-configuration`; fallback `{issuer}/security/token`.
...
- **`lib/api/client.ts`:**
  - **`requireAccessToken()`** throws **`HclCsApiError`** with status `401` if there is no session or no `session.accessToken`.
  - All HCL.CS API calls (e.g. `hclCsPost`, `hclCsGet`) that get a 401 from the server throw `HclCsApiError` with `statusCode: 401`.
- **`apiErrorMessage(error)`:** For `HclCsApiError` with status 401, returns **"Session expired. Please sign in again."**
- **`getLoadErrorInfo(error)`:** Returns `{ message, isUnauthorized }`; `isUnauthorized` is true when the error is a 401 `HclCsApiError` (via **`isUnauthorizedError()`**).
...
1. Calls **`logoutAction()`** (server action): HCL.CS SignOut API and revoke access + refresh tokens. Errors are ignored so the client always proceeds.
...
- SignInAgainButton forcing a full logout (HCL.CS + NextAuth) and a full navigation to `/login`.

### 4.6 Logout and Revocation

- **`app/actions/logout.ts` — `logoutAction()`:**
  - Gets session via `auth()`.
  - If session has tokens: calls HCL.CS SignOut API (`signOut()`), then revokes access token and, if present, refresh token via **`lib/api/revocation.ts`** (`revokeToken`).
...
So both "Sign in again" (expired session) and normal "Logout" ensure the HCL.CS session and tokens are revoked when possible, and the NextAuth cookie is always cleared.
...
| **Session expiry (401)** | `lib/api/client.ts` — 401 → `HclCsApiError`, `apiErrorMessage()`, `getLoadErrorInfo()`, `isUnauthorizedError()`. |
...
- **Session and auth integration (including logout):** `HCL.CS-admin/docs/session-and-auth-integration.md`
- **Google login plan:** `docs/auth/google-login-hcl-cs-admin-plan.md`
- **Demo Server debugging:** `docs/HCL.CS.Demo.Server_Debugging_Guide.md`

This document and the above together give a full picture of HCL.CS-Admin authentication, Demo Server login (including Google), session maintenance, and session expiry handling with implementation pointers.
