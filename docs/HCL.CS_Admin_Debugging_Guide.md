...
# HCL.CS Admin - Debugging & Development Guide
...
| **Project Path** | `HCL.CS-admin/` |
| **Framework** | Next.js 14.2.5 |
| **React Version** | 18.3.1 |
| **Authentication** | NextAuth.js 4.24.7 |
| **Language** | TypeScript 5.5.4 |
| **Purpose** | Administrative UI for HCL.CS Identity Server |
...
│  │  ┌─────────────┐  ┌─────────────┐  ┌─────────────┐  ┌────────────┐ │   │
│  │  │ Credentials │──│   JWT       │──│  Session    │──│  HCL.CS    │ │   │
...
**File:** `HCL.CS-admin/app/layout.tsx`
...
**File:** `HCL.CS-admin/app/api/auth/[...nextauth]/route.ts`
...
- `/api/auth/callback/hcl-cs` - OAuth callback
  ...
  providers: [
    CredentialsProvider({
      ...
      authorize: async (credentials) => {
        // Password grant to HCL.CS server
...
Name: HCL.CS Admin - Debug
URL: https://localhost:3000
Remote URLs:
    http://localhost:3000 → file:///path/to/HCL.CS-admin
```
...
```json
{
  ...
  "configurations": [
    ...
    },
    {
      ...
      "request": "launch",
      "url": "https://localhost:3000",
      "webRoot": "${workspaceFolder}/HCL.CS-admin"
    },
    {
...
**Common Issues:**
- Wrong HCL_CS_ISSUER URL
- Client not registered in HCL.CS
...
| `/api/auth/callback/hcl-cs` | GET/POST | OAuth callback |
...
### HCL.CS API Endpoints (called from lib/api/*)
...
# Run without HTTPS (not recommended for HCL.CS)
npm run dev
...
4. HCL.CS server accepts the token issuer
```
...
```bash
# Check:
1. User has admin role in HCL.CS
...
3. `GET /Security/Api/*` - API calls to HCL.CS
