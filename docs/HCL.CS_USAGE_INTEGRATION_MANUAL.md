...
1. [Introduction to HCL.CS](#1-introduction-to-hcl-cs)
2. [Core Features Overview](#2-core-features-overview)
3. [Architecture Overview](#3-architecture-overview)
4. [How to Setup HCL.CS](#4-how-to-setup-hcl-cs-step-by-step)
...
## 1. Introduction to HCL.CS

### 1.1 What is HCL.CS?

HCL.CS is an OAuth 2.0 and OpenID Connect (OIDC) authorization server. It authenticates users and issues security tokens that applications use to access protected resources.

Think of HCL.CS as a security checkpoint:
- Users prove their identity to HCL.CS
- HCL.CS issues a token (like a temporary badge)
- Applications present this token to access APIs
- APIs validate the token with HCL.CS before allowing access

### 1.2 What Problem Does HCL.CS Solve?

**Without HCL.CS (The Problem):**
```
┌─────────┐     ┌─────────┐     ┌─────────┐
│  App A  │     │  App B  │     │  App C  │
│  ─────  │     │  ─────  │     │  ─────  │
...
**What it does:**
The most secure OAuth flow for web applications. User authenticates with HCL.CS, HCL.CS redirects back with a code, app exchanges code for tokens.
...
2. App redirects to HCL.CS with PKCE parameters
3. User enters credentials on HCL.CS
4. HCL.CS redirects back with authorization code
...
2. Service A authenticates with HCL.CS using client ID + secret
3. HCL.CS returns access token
...
**What JWT does:**
JSON Web Tokens contain claims (user info, permissions) signed by HCL.CS. APIs can validate them without calling HCL.CS.
...
│  ─────────────────────                                                      │
│                                                                             │
│  User ──► Browser ──► /authorize ──► HCL.CS ──► Login Page                 │
│                               (with PKCE)                                   │
│                                                                             │
│  User enters credentials                                                    │
│                                                                             │
│  HCL.CS ──► Redirect ──► Browser ──► /callback ──► Client App              │
│                    (with authorization code)                                │
│                                                                             │
│  STEP 2: TOKEN EXCHANGE                                                     │
│  ──────────────────────                                                     │
│                                                                             │
│  Client App ──► POST /token ──► HCL.CS                                     │
│              (code + code_verifier + client_secret)                         │
│                                                                             │
│  HCL.CS ──► Response ──► Client App                                        │
│              {                                                              │
...
## 4. How to Setup HCL.CS (Step-by-Step)
...
```bash
# Required
export HCL_CS_DB_CONNECTION_STRING="Server=localhost;Database=HclCsDev;User Id=sa;Password=YourPassword;"
export HCL_CS_TOKEN_ISSUER_URI="https://localhost:5001"

# Optional (self-signed certs will be auto-generated if not provided)
export HCL_CS_RSA_SIGNING_CERT_BASE64=""
export HCL_CS_ECDSA_SIGNING_CERT_BASE64=""
export HCL_CS_SIGNING_CERT_PASSWORD=""
```
...
```bash
cd installer/HCL.CS.Installer.Mvc
dotnet run
...
```bash
# For SQL Server
dotnet ef database update --project src/Identity/HCL.CS.Identity.Persistence

# Or use provided SQL scripts
cd scripts/seed/Sql
sqlcmd -S localhost -d HclCsDev -i HclCsSqlV1.sql
```
...
|----------|-------------|---------|
| `HCL_CS_DB_CONNECTION_STRING` | Database connection | `Server=prod-db;Database=HCL.CS;...` |
| `HCL_CS_TOKEN_ISSUER_URI` | Public URL of HCL.CS | `https://auth.yourcompany.com` |
| `HCL_CS_RSA_SIGNING_CERT_BASE64` | RSA certificate (Base64) | `MIIFXTCCA0WgAwIBAgIQC...` |
| `HCL_CS_ECDSA_SIGNING_CERT_BASE64` | ECDSA certificate (Base64) | `MIIBkTCB+wIJAKHBfpE...` |
| `HCL_CS_SIGNING_CERT_PASSWORD` | Certificate password | `SecurePassword123!` |
| `HCL_CS_RSA_SIGNING_KID` | RSA key identifier | `hcl-cs-rsa-2026-q1` |
| `HCL_CS_ECDSA_SIGNING_KID` | ECDSA key identifier | `hcl-cs-ecdsa-2026-q1` |
...
3. Update `HCL.CS_*_SIGNING_KID` to new key
...
Edit `demos/HCL.CS.Demo.Server/Program.cs`:
...
│  • Stored in database: HclCs_SecurityTokens                               │
...
• Cannot forge tokens
• HCL.CS private key stays secure
```
...
public async Task<IActionResult> Logout()
{
    ...
    // 3. Redirect to HCL.CS logout (ends SSO session)
    return SignOut(
...
const logout = async () => {
  ...
  // 2. Sign out (redirects to HCL.CS logout page)
...
export HCL_CS_RSA_SIGNING_CERT_BASE64=$(base64 new-rsa-cert.pem)
export HCL_CS_RSA_SIGNING_KID="hcl-cs-rsa-2026-q2"
```
...
grep "TokenReuseDetected" /var/log/hcl-cs/app.log

# Find failed auth by IP
grep "Authentication failed" /var/log/hcl-cs/app.log | \
  awk '{print $5}' | sort | uniq -c | sort -rn | head -10

# Find rate limit hits
grep "Rate limit exceeded" /var/log/hcl-cs/app.log
```
...
**Configuration:**
- HCL.CS connects to Active Directory
...
{
    ValidAudience = "yourcompany.api", // Must match HCL.CS config
    // ...
};
```
...
# Query HCL.CS database:
SELECT * FROM HclCs_SecurityTokens 
...
### Does HCL.CS support multi-tenancy?

**Answer:** Not natively. Each tenant requires a separate HCL.CS instance or you can implement tenant isolation using:
...
```bash
export HCL_CS_RSA_SIGNING_CERT_BASE64=$(base64 your-cert.pfx)
export HCL_CS_SIGNING_CERT_PASSWORD="your-password"
```
...
**Answer:** For many use cases, yes. Consider HCL.CS if:
...
2. Linking external accounts to HCL.CS users
3. Using HCL.CS for session management
...
- Modify views in `demos/HCL.CS.Demo.Server/Views/`
- Add CSS/JS to `wwwroot/`
- Or build your own login UI using HCL.CS APIs

### How do I backup HCL.CS?
...
- Building custom admin interface using HCL.CS APIs
...
- [ ] `HCL_CS_SIGNING_CERT_PASSWORD` is strong (>32 chars, random)
...
**Version:** 1.0  
**Last Updated:** February 2026  
**Maintained By:** HCL.CS Documentation Team  
**Feedback:** Submit issues to documentation repository

---

*This manual provides practical guidance for integrating with and deploying HCL.CS. For additional support, refer to the API reference documentation or contact the development team.*
