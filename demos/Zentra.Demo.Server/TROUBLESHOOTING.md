# Zentra Demo Server - Troubleshooting Guide
...
3. Restart the server:
   ```bash
   dotnet run --project Zentra.DemoServerApp.csproj
   ```
...
```bash
dotnet run --project Zentra.DemoServerApp.csproj --urls "https://localhost:5002"
```
...
```bash
# From the Zentra.Demo.Server directory
dotnet run --project Zentra.DemoServerApp.csproj
