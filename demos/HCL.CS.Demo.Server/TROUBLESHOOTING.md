...
# HCL.CS Demo Server - Troubleshooting Guide
...
3. Restart the server:
   ```bash
   dotnet run --project HCL.CS.DemoServerApp.csproj
   ```
...
```bash
dotnet run --project HCL.CS.DemoServerApp.csproj --urls "https://localhost:5002"
```
...
```bash
# From the HCL.CS.Demo.Server directory
dotnet run --project HCL.CS.DemoServerApp.csproj
