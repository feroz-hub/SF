...
IF NOT EXISTS (
    ...
    WHERE [MigrationId] = N'20220722123632_HclCsSqlV1'
)
BEGIN
...
BEGIN
    EXEC(N'CREATE UNIQUE INDEX [IX_CLI_CLID_CLSEC] ON [HclCs_Clients] ([ClientId], [ClientSecret]) WHERE [ClientId] IS NOT NULL AND [ClientSecret] IS NOT NULL');
END;
GO
...
BEGIN
    ...
    VALUES (N'20220722123632_HclCsSqlV1', N'8.0.11');
END;
GO
...
IF NOT EXISTS (
    SELECT * FROM [__EFMigrationsHistory]
    WHERE [MigrationId] = N'20260726060000_Phase2HclIdentityProfile'
)
BEGIN
    ALTER TABLE [HclCs_Users] ADD [DirectoryImmutableId] nvarchar(255) NULL;
END;
GO
...
BEGIN
    ALTER TABLE [HclCs_Users] ADD [EmployeeId] nvarchar(255) NULL;
END;
GO
...
BEGIN
    ALTER TABLE [HclCs_Users] ADD [UserPrincipalName] nvarchar(255) NULL;
END;
GO
...
BEGIN
    ALTER TABLE [HclCs_Users] ADD [DisplayName] nvarchar(255) NULL;
END;
GO
...
BEGIN
    ALTER TABLE [HclCs_Users] ADD [Department] nvarchar(255) NULL;
END;
GO
...
BEGIN
    ALTER TABLE [HclCs_Users] ADD [AuthenticationSource] nvarchar(255) NULL;
END;
GO
...
BEGIN
    ALTER TABLE [HclCs_Users] ADD [DirectoryLastValidatedAt] datetimeoffset NULL;
END;
GO
...
BEGIN
    INSERT INTO [__EFMigrationsHistory] ([MigrationId], [ProductVersion])
    VALUES (N'20260726060000_Phase2HclIdentityProfile', N'8.0.11');
END;
GO
...
IF NOT EXISTS (
    SELECT * FROM [__EFMigrationsHistory]
    WHERE [MigrationId] = N'20260727130000_Phase2BLocalAuthenticationEmailUniqueness'
)
BEGIN
    DROP INDEX [EmailIndex] ON [HclCs_Users];
END;
GO
...
BEGIN
    EXEC(N'CREATE UNIQUE INDEX [EmailIndex] ON [HclCs_Users] ([NormalizedEmail]) WHERE [NormalizedEmail] IS NOT NULL');
END;
GO
...
BEGIN
    INSERT INTO [__EFMigrationsHistory] ([MigrationId], [ProductVersion])
    VALUES (N'20260727130000_Phase2BLocalAuthenticationEmailUniqueness', N'8.0.11');
END;
GO
...
IF NOT EXISTS (
    SELECT * FROM [__EFMigrationsHistory]
    WHERE [MigrationId] = N'20260728140000_Phase2CCoreInfrastructureSchema'
)
BEGIN
...
BEGIN
    ALTER TABLE [HclCs_SecurityTokens] ADD [TokenReuseDetected] bit NOT NULL DEFAULT CAST(0 AS bit);
END;
GO
...
BEGIN
    ALTER TABLE [HclCs_Clients] ADD [PreferredAudience] nvarchar(300) NULL;
END;
GO
...
BEGIN
    CREATE TABLE [HclCs_ExternalIdentities] (
        ...
        CONSTRAINT [FK_HclCs_ExternalIdentities_HclCs_Users_UserId] FOREIGN KEY ([UserId]) REFERENCES [HclCs_Users] ([Id]) ON DELETE NO ACTION
    );
END;
GO
...
BEGIN
    CREATE TABLE [HclCs_ExternalAuthProviderConfig] (
        ...
        [ConfigJson] nvarchar(max) NOT NULL,
        [AutoProvisionEnabled] bit NOT NULL DEFAULT CAST(0 AS bit),
...
BEGIN
    EXEC(N'CREATE UNIQUE INDEX [IX_EXTID_PROVIDER_ISSUER_SUBJECT] ON [HclCs_ExternalIdentities] ([Provider], [Issuer], [Subject]) WHERE [Provider] IS NOT NULL AND [Issuer] IS NOT NULL AND [Subject] IS NOT NULL');
END;
GO
...
BEGIN
    EXEC(N'CREATE UNIQUE INDEX [IX_EAPC_PROVIDER] ON [HclCs_ExternalAuthProviderConfig] ([ProviderName]) WHERE [ProviderName] IS NOT NULL');
END;
GO
...
BEGIN
    INSERT INTO [__EFMigrationsHistory] ([MigrationId], [ProductVersion])
    VALUES (N'20260728140000_Phase2CCoreInfrastructureSchema', N'8.0.11');
END;
GO
