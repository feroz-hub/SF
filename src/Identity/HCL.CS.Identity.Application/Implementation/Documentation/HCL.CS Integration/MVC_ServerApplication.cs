...
* <p><strong>The HCL.CS needs to be added to the services collection of the MVC Server application in the Startup class using the following code.</strong></p>
* \code
    services.AddHclCs(systemSettings, tokenSettings, new NotificationTemplateSettings())
            .AddAsymmetricKeystore(LoadAsymmetricKey());
* \endcode
* <ul><li>The services.AddHclCs() method has three parameters which are the System settings, tokensettings and notification template settings.</li>
...
* \endcode
* <p>To sum it up, the Configure services collection for adding the HCL.CS Extension looks like below  by passing the server objects to the AddHclCs() method.</strong></p>
\code
        public void ConfigureServices(IServiceCollection services)
        {
            SystemSettings systemSettings = new SystemSettings();
            systemSettings.DBConfig.Database = DbTypes.SqlServer;
            systemSettings.DBConfig.DBConnectionString = "Server=localhost\\SQLEXPRESS;Database=HclCsFrameworkToken;Trusted_Connection=True;MultipleActiveResultSets=true";
...
* \endcode
* <para>The AddHclCs() method can also be added by passing the JSON to the method as shown below</para>
* \code
        var systemSettingPath = @"C:\Source\HCL.CS-Dev\IntegrationTests\Configurations\SystemSettings.json";
        var tokenConfigPath = @"C:\Source\HCL.CS-Dev\IntegrationTests\Configurations\TokenSettings.json";
        var notificationTemplatePath = @"C:\Source\HCL.CS-Dev\IntegrationTests\Configurations\NotificationTemplateSettings.json";
        services.AddHclCs(systemSettingPath, tokenConfigPath, notificationTemplatePath);
* \endcode
* <p>Similarly, the Configure() method should add the middleware of HCL.CS as in the code shown below:</p>
* \code
