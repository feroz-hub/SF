...
* </ol>
* <p><strong> Configuring HCL.CS for a Web application client : </strong></p>
* <p>The following process is done for a web app client to configure the HCL.CS for its OAuth 2.0 features to be utilized.</p>
* <ul>
...
* <li>The user is redirected back to the application with an authorization code in the query string (redirect url). This is necessary as some flows supported by the HCL.CS needs the client application to get the code and process the same by posting it to the endpoints as necessary. For e.g. in the authorization code flow, the authorization code is given to the application back via a redirect url and then this code is again send back to the token endpoint to get the access/identity token.</li>
...
* <p>The HCL.CS (authorization server) will redirect the browser back to the <strong> redirect_uri </strong> specified by the application, adding a&nbsp;code&nbsp;and&nbsp;state&nbsp;to the query string.</p>
...
* <p>The <strong> code </strong> is the authorization code generated by the authorization server. This code is relatively short-lived, typically lasting between 1 to 10 minutes depending on the configuration doen in the HCL.CS.</p>
...
* <li>For each registered client in the server that has a session for the logged-in user from the HCL.CS, an iframe is rendered to the browser. This means that logout requests of all clients of the same logged-in user are performed in parallel.</li>
* </ul>
* <p>The HCL.CS adds the below query parameters when rendering the logout URI from the front channel.</p>
* <table>
* <tr>
* <td>
...
* </td>
* <td>
* <p>Issuer Identifier URL for the authentication provider (HCL.CS) issuing the front-channel logout request.</p>
* </td>
* </tr>
* <tr>
* <td>
...
* <li>HCL.CS supports both front channel and back-channel logouts. This is specified in the discovery end point.</li>
* </ul>
...
* <li>After logout, the HCL.CS triggers a logout at other clients using the front- or back-channel logout mechanism or a combination of both based on the configuration.</li>
...
* <p>Logout tokens are JWT Tokens which are a back-channel mechanism for notifying subscribed client applications that an end-user has been logged out of the HCL.CS(authentication provider).</p>
* <ul>
* <li>The logout can be explicit, or result from the expiration of the end-user session with the identity provider(HCL.CS)</li>
* <li>An application receiving a logout token at the notification URI must validate its type, signature and claims to ensure the token originates from the authentication provider(HCL.CS) and is not a&nbsp; forgery.</li>
...
* <li>Network failures or internal server errors in HCL.CS, or HCL.CS is temporarily unavailable</li>
...
* <p>In scenario (2), the HCL.CS will send an error response in one of the following ways:</p>
* <ul>
