...
using HCL.CS.Infrastructure.Data.UnitOfWork.Api;
using HCL.CS.Infrastructure.Data.UnitOfWork.Endpoint;
using HCL.CS.Infrastructure.Data.Validation;
using HCL.CS.Infrastructure.Data.Wrappers;

namespace HCL.CS.Infrastructure.Data.Extension;
...
public static class InfrastructureDataExtension
{
    ...
    private static HclCsConfig GetRegisteredConfiguration(IServiceCollection services)
    {
        var configDescriptor = services.LastOrDefault(d => d.ServiceType == typeof(HclCsConfig));
        var configSettings = configDescriptor?.ImplementationInstance as HclCsConfig;
        if (configSettings == null)
            throw new InvalidOperationException("HCL.CS configuration has not been registered.");
    ...
    private static IServiceCollection AddIdentityServices(IServiceCollection services, HclCsConfig config)
    {
