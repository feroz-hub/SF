...
using Zentra.Service.Implementation.Endpoint.Comparers;
using Zentra.Service.Implementation.Endpoint.Converters;
...
internal static class HttpContextExtension
{
    ...
    internal static void RedirectToAbsoluteUrl(this HttpResponse response, string url)
    {
        if (url.CheckLocalUrl())
        {
            ...
            var host = response.HttpContext.GetZentraBaseUrl().IncludeEndSlash();
    ...
    internal static string GetZentraBaseUrl(this HttpContext context)
    {
        return context.GetZentraHost() + context.GetZentraBasePath();
    }

    internal static string GetZentraHost(this HttpContext context)
    {
        var request = context.Request;
        return request.Scheme + "://" + request.Host.ToUriComponent();
    }

    internal static void SetZentraBasePath(this HttpContext context, string value)
    {
        if (context != null) context.Items[AuthenticationConstants.EnvironmentPaths.ZentraBasePath] = value;
    }

    internal static string GetZentraBasePath(this HttpContext context)
    {
        return context.Items[AuthenticationConstants.EnvironmentPaths.ZentraBasePath] as string;
    }

    internal static string GetZentraRelativePath(this HttpContext context, string path)
    {
        if (!path.IsLocalUrl()) return null;

        if (path.StartsWith("~/")) path = path.Substring(1);

        path = context.GetZentraBaseUrl().IncludeEndSlash() + path.RemoveFrontSlash();
        return path;
    }
