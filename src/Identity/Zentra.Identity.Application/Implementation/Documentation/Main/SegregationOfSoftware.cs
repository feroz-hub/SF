...
* <li><strong> Resources: </strong> It consist of resource files which hold validation messages and constants used everywhere. It also holds the configuration settings, token settings that are required for the Zentra.</li>
* <li><strong> Services: </strong> It contains the implementation for Logger, Audit Trail and Notification Services. These components will be used within Zentra as well target user applications.
* <ul>
* <li><strong> Logger: </strong> It is developed using Serilog, it will be used across Zentra. This module will log all user or system activities in file/database based on the configuration. Log file size and location are controlled through logger configuration parameters.</li>
...
* <li><strong> Notifications: </strong> It is used to send an email and SMS notifications from Zentra. Notification can be triggered by framework (scenarios such account creation, account verification etc.) and invoked by target application based on business requirements. It supports sending notification based on predefined/custom template.</li>
* </ul>
* </li>
* </ul>
*/
