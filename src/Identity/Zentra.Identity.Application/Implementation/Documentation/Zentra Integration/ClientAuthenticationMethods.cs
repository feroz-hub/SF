...
* <p>Zentra supports one client authentication method for confidential clients. Authentication is performed once the endpoint is invoked from the client application.</p>
* <p>When a client is registered, the client id and client secret is given to the client application. When the client requests a token, it embeds the client id and client secret in the request and the Zentra will check for its validity in the back-end.</p>
...
* </ul>
* <p>The client without any authentication method (none) is not supported in the Zentra due to supporting only confidential clients</p>
* <p><strong> Client Types </strong></p>
* <p>Confidential clients are applications that are able to securely authenticate with the Zentra so as to keep their registered client secret safe.</p>
...
* <p>The Zentra considers all clients that are registered as <strong> confidential </strong> clients. So client secret is mandatory for all endpoint requests except authorize code flow during the login process.</p>
* <p><strong> Public </strong> clients are not supported by the Zentra.</p>
*/
