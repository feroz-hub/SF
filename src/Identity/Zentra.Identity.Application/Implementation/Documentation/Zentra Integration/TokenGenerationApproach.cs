...
* <p>The token is generated in the Zentra based on the input parameters specified in the request of the authorize endpoint (hybrid flow) and the token endpoint (all other flows - authorize code, client_credentials, refresh_token and the resource owner password.)</p>
...
* <p>The scope input is matched with identity resources configured in the Zentra. The identity claims that matches the identity resource are taken and the claims are created with identity resource claim type and actual value as claim value from the user table. Refer to \ref CustomClaimsinIdentityToken "Custom Claims in Identity Token"</p>
* </td>
* </tr>
* <tr>
* <td>
...
* </table>
* <p><strong> Custom Claims :</strong></p>
* <p><span style="background-color: transparent;">The scope input parameter string when matches the ApiResource, its ApiResourceClaims will be searched for Zentra defined claimed types and the matching claim values in the RoleClaims/UserClaims will be added.&nbsp; Refer to \ref CustomClaimsAccessToken "Custom Claims in access token"</span></p>
* </td>
* </tr>
* <tr>
* <td>
...
* <li>The refresh token is first stored in the database as a 32 bit random string, so as that the revocation and the renew of the refresh token can be done by the Zentra.</li>
...
* <li>When current access tokens expire or become invalid, the Zentra provides refresh tokens to the client to obtain new access token through the refresh_token flow.</li>
* <li>The normal refresh token lifetime is set as 1 day by default in the Zentra. (86400 seconds) whereas the access token lifetime is set as 1 hour. (3600 seconds)</li>
* <li>During the refresh token renewal process, the old refresh token is hard deleted from the database and the new 32 bit random string is created with the existing payload of the access token.</li>
