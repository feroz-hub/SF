...
public class HclCsEndpointMiddleware
{
    private readonly ILoggerService loggerService;
    private readonly RequestDelegate next;
    private readonly TokenSettings tokenSettings;

    public HclCsEndpointMiddleware(RequestDelegate next, ILoggerInstance instance, HclCsConfig configSettings)
    {
        this.next = next;
    ...
    {
        SetHclCsBasePath(httpContext);
        SetHclCsLoggedUser(httpContext);
    ...
    private void SetHclCsBasePath(HttpContext httpContext)
    {
        var path = httpContext.Request.PathBase.Value;
        if (path != null && path.EndsWith("/")) path = path[..^1];

        httpContext.Items[AuthenticationConstants.EnvironmentPaths.HclCsBasePath] = path;
    }

    private void SetHclCsLoggedUser(HttpContext httpContext)
    {
