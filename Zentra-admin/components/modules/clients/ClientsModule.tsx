...
import { type ClientsModel } from "@/lib/types/zentra";
...
const zentraGroupLabels: Record<string, string> = {
  client: "Clients",
...
function buildScopeGroups(allScopes: string[]): ScopeGroup[] {
  ...
  for (const scope of allScopes) {
    ...
    if (scope.toLowerCase().startsWith("zentra.")) {
      const rest = scope.substring("zentra.".length);
      const [resourceKey, ...permissionParts] = rest.split(".");
      const key = resourceKey || "other";
      const groupId = `zentra:${key.toLowerCase()}`;
      const label = zentraGroupLabels[key.toLowerCase()] ?? `Zentra: ${key}`;
    ...
    <div className="form-grid">
      <div className="form-grid" style={{ gap: "0.6rem" }}>
        ...
        <div className="form-row">
          ...
          />
          <p className="inline-message">Human-friendly name shown in Zentra and consent screens (max {MAX_CLIENT_NAME_LENGTH} characters).</p>
        </div>
      ...
      <div className="form-grid" style={{ gap: "0.6rem" }}>
        ...
        <div className="form-row">
          ...
          <Textarea
            ...
            placeholder="https://localhost:3000/api/auth/callback/zentra"
        ...
        <div className="form-row">
          ...
          <Input
            ...
            placeholder="e.g. rentflow.api or zentra.api"
