...
export function UsersModule({ rows, loadError, loadErrorIsUnauthorized }: Props) {
  ...
  useEffect(() => {
    ...
    try {
      const raw = window.localStorage.getItem("zentra-users-views");
      if (!raw) return;
  ...
  const saveCurrentView = () => {
    ...
    try {
      ...
      window.localStorage.setItem("zentra-users-views", JSON.stringify(parsed));
      setSavedViews(Object.keys(parsed));
      setActiveView(name);
  ...
  return (
    <>
      ...
      <Dialog
        ...
        description="How users, roles, and assignments work in Zentra."
