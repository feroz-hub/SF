...
function relativeToZentraAdmin(...segments) {
...
function main() {
  const repoRootFromZentraAdmin = relativeToZentraAdmin("..");

  const openApiPath = path.resolve(repoRootFromZentraAdmin, "docs", "api", "all-apis.openapi.json");
  const mdPath = path.resolve(repoRootFromZentraAdmin, "docs", "api", "all-apis.md");
  const routesPath = relativeToZentraAdmin("lib", "api", "routes.ts");
  ...
  printList("[api-coverage] MISSING in Zentra-admin route map", missing);
  printList("[api-coverage] EXTRA in Zentra-admin route map (not in docs)", extra);
