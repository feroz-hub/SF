...
export async function listClientActiveTokens(
  ...
  return zentraPostWithSession<TokenModel[], ClientTokenLookupRequest>(ApiRoutes.securityToken.getByClientIds, payload);
}
...
export async function listTokensByDateRange(
  ...
  return zentraPost<TokenModel[], DateRangeLookupRequest>(route, payload, accessToken);
}
...
export async function revokeToken(token: string, tokenTypeHint: "access_token" | "refresh_token") {
  ...
  const response = await zentraFetch(endpoint, {
    method: "POST",
    headers: {
