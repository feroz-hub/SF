...
export async function searchAudit(input: AuditQueryInput): Promise<AuditResponseModel | null> {
  ...
  const result = await zentraPostWithSession<AuditResponseModel | null, AuditSearchRequestModel>(
...
export async function addAuditTrail(payload: unknown): Promise<unknown> {
  return zentraPostWithSession<unknown, unknown>(ApiRoutes.audit.addAuditTrail, payload);
}

export async function addAuditTrailModel(payload: unknown): Promise<unknown> {
  return zentraPostWithSession<unknown, unknown>(ApiRoutes.audit.addAuditTrailModel, payload);
}
