...
import { zentraGetWithSession, zentraPostWithSession } from "@/lib/api/client";
import { env } from "@/lib/env";
import { zentraFetch } from "@/lib/server-fetch";

async function fetchText(url: string, init?: RequestInit): Promise<string> {
  const response = await zentraFetch(url, { ...init, cache: "no-store" });
...
export const Operations = {
  oidc: {
    jwks: async () => zentraGetWithSession<unknown>(ApiRoutes.endpoint.jwks),
    discovery: async () => zentraGetWithSession<unknown>(ApiRoutes.endpoint.discovery),
    token: async (payload: unknown) => zentraPostWithSession<unknown, unknown>(ApiRoutes.endpoint.token, payload),
    introspect: async (payload: unknown) => zentraPostWithSession<unknown, unknown>(ApiRoutes.endpoint.introspect, payload),
    userinfo: async (payload: unknown) => zentraPostWithSession<unknown, unknown>(ApiRoutes.endpoint.userinfo, payload),
    revocation: async (payload: unknown) => zentraPostWithSession<unknown, unknown>(ApiRoutes.endpoint.revocation, payload),
  ...
  health: {
    health: async () => zentraGetWithSession<unknown>(ApiRoutes.health.health),
    live: async () => zentraGetWithSession<unknown>(ApiRoutes.health.live),
    ready: async () => zentraGetWithSession<unknown>(ApiRoutes.health.ready)
  },
  installer: {
  ...
  demoExternalAuth: {
    ...
    linkGoogle: async (payload: unknown) =>
      zentraPostWithSession<unknown, unknown>(ApiRoutes.demoExternalAuth.linkGoogle, payload, env.demoServerBaseUrl),
    unlinkGoogle: async (payload: unknown) =>
      zentraPostWithSession<unknown, unknown>(ApiRoutes.demoExternalAuth.unlinkGoogle, payload, env.demoServerBaseUrl)
  }
} as const;
