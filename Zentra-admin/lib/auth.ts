...
async function executePasswordGrantAttempt(
  ...
  const response = await zentraFetch(tokenEndpoint, {
    method: "POST",
    headers,
...
async function requestPasswordGrantTokens(userName: string, password: string): Promise<TokenEndpointResponse> {
  ...
  for (const attempt of scopedAttempts) {
    ...
    if (result.ok) {
      console.info(`[zentra-auth] Password grant succeeded (scopes=included, clientSecretPost=${attempt.useClientSecretPost}).`);
      return result.token;
    }

    lastErrorCode = result.errorCode;
    lastErrorDescription = result.errorDescription;
    console.warn(`[zentra-auth] Password grant attempt failed (scopes=included, clientSecretPost=${attempt.useClientSecretPost}): ${result.errorCode} — ${result.errorDescription}`);
  ...
  if (shouldTryScopeless) {
    console.warn("[zentra-auth] Scoped attempts failed. Trying without scopes — token may lack required admin API access.");
    for (const attempt of scopelessAttempts) {
      const result = await executePasswordGrantAttempt(tokenEndpoint, userName, password, attempt);
      if (result.ok) {
        console.warn(`[zentra-auth] Password grant succeeded WITHOUT scopes (clientSecretPost=${attempt.useClientSecretPost}). Token may lack zentra.* scopes needed for admin APIs.`);
        return result.token;
      }
