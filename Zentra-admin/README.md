# Zentra Admin (Next.js)

This project is a Next.js admin client for Zentra, with login behavior aligned to MVC:
...
Important: Zentra validates redirect URLs as HTTPS. Run this app over `https://localhost:3000`.
...
- `ZENTRA_AUTHORITY`
- `ZENTRA_CLIENT_ID`
- `ZENTRA_CLIENT_SECRET`

## 2) Installer URIs for this client

Use these in Zentra installer/client config:

- Redirect URI: `https://localhost:3000/api/auth/callback/zentra`
...
Set `ZENTRA_ENABLE_FEDERATED_LOGOUT=true` to clear IdP session via Zentra `/security/endsession`.
