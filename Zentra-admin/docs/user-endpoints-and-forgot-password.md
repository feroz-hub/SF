...
## 2. Forgot Password flow in Zentra-admin
...
3. **Email**  
   - User receives email from Zentra (SMTP) with the reset token (or link containing it), per server notification config.
...
- **Anonymous calls:** `lib/api/client.ts` exposes `zentraPostAnonymous(path, payload)` (no `Authorization` header). Used by `lib/api/passwordReset.ts` for `generatePasswordResetTokenAnonymous` and `resetPasswordAnonymous`.
