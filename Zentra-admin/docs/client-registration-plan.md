# Client registration: server flow, required fields, validation, and Zentra-admin alignment
...
```
Zentra-admin (Create Client)  →  POST /Security/Api/Client/RegisterClient  (body: ClientsModel)
       ↓
...
| Step | Location |
|------|----------|
| API route | [Gateway/Routes/ClientServiceRoute.cs](src/Gateway/Zentra.Gateway/Routes/ClientServiceRoute.cs) – `RegisterClient` |
| Route path | [ApiRoutePathConstants.RegisterClient](src/Identity/Zentra.Identity.Domain/Constants/ApiRoutePathConstants.cs) = `"/Security/Api/Client/RegisterClient"` |
| Service | [ClientService.RegisterClientAsync](src/Identity/Zentra.Identity.Application/Implementation/Api/Services/ClientService.cs) |
| Validation | [ClientModelSpecification](src/Identity/Zentra.Identity.Application/Implementation/Api/Specifications/ClientModelSpecification.cs) (CrudMode.Add) |
| Model | [ClientsModel](src/Identity/Zentra.Identity.Domain/Models/Endpoint/ClientsModel.cs) |
...
All rules below run for **Add** (create). Rules are in [ClientModelSpecification.cs](src/Identity/Zentra.Identity.Application/Implementation/Api/Specifications/ClientModelSpecification.cs).
...
From [TokenConfig.cs](src/Identity/Zentra.Identity.Domain/Configurations/Endpoint/TokenConfig.cs) (defaults; can be overridden by config):
...
## 3. Zentra-admin: create-client payload and validation

### 3.1 Where it happens

- **Form and submit:** [components/modules/clients/ClientsModule.tsx](Zentra-admin/components/modules/clients/ClientsModule.tsx) – Create dialog, `onSubmitCreate` → `createClientAction(toPayload(form))`.
- **Action and model:** [app/admin/clients/actions.ts](Zentra-admin/app/admin/clients/actions.ts) – `formSchema.safeParse(input)`, then `buildCreateModel(parsed.data, actor)`.
- **API:** [lib/api/clients.ts](Zentra-admin/lib/api/clients.ts) – `createClient(model)` → POST `ApiRoutes.client.registerClient` with body `ClientsModel`.
...
- **CreatedBy:** actor (name or email or `"zentra-admin"`)  
...
## 5. Recommended checks in Zentra-admin (implemented)
