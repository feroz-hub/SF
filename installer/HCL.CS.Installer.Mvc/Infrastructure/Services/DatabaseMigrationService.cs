...
public sealed class DatabaseMigrationService : IDatabaseMigrationService
{
        ...
        CancellationToken cancellationToken)
    {
        var statements = provider switch
        {
            ...
            {
                "IF COL_LENGTH('HclCs_SecurityTokens', 'ConsumedAt') IS NULL ALTER TABLE [HclCs_SecurityTokens] ADD [ConsumedAt] datetime2 NULL;",
                "IF COL_LENGTH('HclCs_SecurityTokens', 'TokenReuseDetected') IS NULL ALTER TABLE [HclCs_SecurityTokens] ADD [TokenReuseDetected] bit NOT NULL CONSTRAINT [DF_HclCs_SecurityTokens_TokenReuseDetected] DEFAULT 0;"
            },
            DatabaseProviderType.MySql => new[]
            {
                "ALTER TABLE `HclCs_SecurityTokens` ADD COLUMN IF NOT EXISTS `ConsumedAt` datetime(6) NULL;",
                "ALTER TABLE `HclCs_SecurityTokens` ADD COLUMN IF NOT EXISTS `TokenReuseDetected` tinyint(1) NOT NULL DEFAULT FALSE;"
            },
            DatabaseProviderType.PostgreSql => new[]
            {
                "ALTER TABLE \"HclCs_SecurityTokens\" ADD COLUMN IF NOT EXISTS \"ConsumedAt\" timestamp without time zone NULL;",
                "ALTER TABLE \"HclCs_SecurityTokens\" ADD COLUMN IF NOT EXISTS \"TokenReuseDetected\" boolean NOT NULL DEFAULT FALSE;"
            },
        ...
        CancellationToken cancellationToken)
    {
        var statements = provider switch
        {
            DatabaseProviderType.SqlServer => new[]
            {
                "IF COL_LENGTH('HclCs_Clients', 'PreferredAudience') IS NULL ALTER TABLE [HclCs_Clients] ADD [PreferredAudience] nvarchar(300) NULL;"
            },
            DatabaseProviderType.MySql => new[]
            {
                "ALTER TABLE `HclCs_Clients` ADD COLUMN IF NOT EXISTS `PreferredAudience` varchar(300) NULL;"
            },
            DatabaseProviderType.PostgreSql => new[]
            {
                "ALTER TABLE \"HclCs_Clients\" ADD COLUMN IF NOT EXISTS \"PreferredAudience\" varchar(300) NULL;"
            ...
            DatabaseProviderType.Sqlite => new[]
            {
                "ALTER TABLE \"HclCs_Clients\" ADD COLUMN \"PreferredAudience\" TEXT NULL;"
            },
            _ => Array.Empty<string>()
        };
        ...
        CancellationToken cancellationToken)
    {
        var statements = provider switch
        {
            ...
            {
                """
                IF OBJECT_ID(N'[HclCs_ExternalIdentities]', N'U') IS NULL
                BEGIN
        ...
        CancellationToken cancellationToken)
    {
        var statements = provider switch
        {
            ...
            {
                """
                IF OBJECT_ID(N'[HclCs_NotificationProviderConfig]', N'U') IS NULL
                BEGIN
                    CREATE TABLE [HclCs_NotificationProviderConfig] (
                        [Id] uniqueidentifier NOT NULL,
                        [IsDeleted] bit NOT NULL CONSTRAINT [DF_HclCs_NPC_IsDeleted] DEFAULT (0),
                        [CreatedOn] datetime2 NOT NULL,
                        [ModifiedOn] datetime2 NULL,
                        [CreatedBy] nvarchar(255) NOT NULL,
                        [ModifiedBy] nvarchar(255) NULL,
                ...
                """,
                """
                IF NOT EXISTS (SELECT 1 FROM sys.indexes WHERE name = 'IX_NPC_CHANNEL_TYPE' AND object_id = OBJECT_ID('HclCs_NotificationProviderConfig'))
                    CREATE INDEX [IX_NPC_CHANNEL_TYPE] ON [HclCs_NotificationProviderConfig] ([ChannelType]);
                """,
                """
                IF NOT EXISTS (SELECT 1 FROM sys.indexes WHERE name = 'IX_NPC_CHANNEL_ACTIVE' AND object_id = OBJECT_ID('HclCs_NotificationProviderConfig'))
            ...
            {
                """
                CREATE TABLE IF NOT EXISTS `HclCs_NotificationProviderConfig` (
                    `Id` char(36) NOT NULL,
                    `IsDeleted` tinyint(1) NOT NULL DEFAULT 0,
                    `CreatedOn` datetime(6) NOT NULL,
                    `ModifiedOn` datetime(6) NULL,
            ...
            DatabaseProviderType.PostgreSql => new[]
            {
                ...
                CREATE INDEX IF NOT EXISTS "IX_NPC_CHANNEL_TYPE"
                    ON "HclCs_NotificationProviderConfig" ("ChannelType");
                """,
                """
                CREATE INDEX IF NOT EXISTS "IX_NPC_CHANNEL_ACTIVE"
                    ON "HclCs_NotificationProviderConfig" ("ChannelType", "IsActive");
                """
            },
            DatabaseProviderType.Sqlite => new[]
            {
        ...
        CancellationToken cancellationToken)
    {
        var statements = provider switch
        {
            ...
            {
                """
                IF OBJECT_ID(N'[HclCs_ExternalAuthProviderConfig]', N'U') IS NULL
                BEGIN
                    CREATE TABLE [HclCs_ExternalAuthProviderConfig] (
                        [Id] uniqueidentifier NOT NULL,
                        [IsDeleted] bit NOT NULL CONSTRAINT [DF_HclCs_EAPC_IsDeleted] DEFAULT (0),
                        [CreatedOn] datetime2 NOT NULL,
                        [ModifiedOn] datetime2 NULL,
                        [CreatedBy] nvarchar(255) NOT NULL,
                        [ModifiedBy] nvarchar(255) NULL,
                        ...
                        [ConfigJson] nvarchar(max) NOT NULL,
                        [AutoProvisionEnabled] bit NOT NULL CONSTRAINT [DF_HclCs_EAPC_AutoProvision] DEFAULT (0),
                ...
                """,
                """
                IF NOT EXISTS (SELECT 1 FROM sys.indexes WHERE name = 'IX_EAPC_PROVIDER' AND object_id = OBJECT_ID('HclCs_ExternalAuthProviderConfig'))
                    CREATE UNIQUE INDEX [IX_EAPC_PROVIDER] ON [HclCs_ExternalAuthProviderConfig] ([ProviderName]);
                """,
                """
                IF NOT EXISTS (SELECT 1 FROM sys.indexes WHERE name = 'IX_EAPC_PROVIDER_ENABLED' AND object_id = OBJECT_ID('HclCs_ExternalAuthProviderConfig'))
            ...
            {
                """
                CREATE TABLE IF NOT EXISTS `HclCs_ExternalAuthProviderConfig` (
                    `Id` char(36) NOT NULL,
                    `IsDeleted` tinyint(1) NOT NULL DEFAULT 0,
                    `CreatedOn` datetime(6) NOT NULL,
                    `ModifiedOn` datetime(6) NULL,
            ...
            DatabaseProviderType.PostgreSql => new[]
            {
                ...
                CREATE UNIQUE INDEX IF NOT EXISTS "IX_EAPC_PROVIDER"
                    ON "HclCs_ExternalAuthProviderConfig" ("ProviderName");
                """,
                """
                CREATE INDEX IF NOT EXISTS "IX_EAPC_PROVIDER_ENABLED"
                    ON "HclCs_ExternalAuthProviderConfig" ("ProviderName", "IsEnabled");
                """
            },
            DatabaseProviderType.Sqlite => new[]
            {
