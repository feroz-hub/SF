...
using ZentraInstallerMVC.ViewModels;

namespace ZentraInstallerMVC.Services;
...
public sealed class InstallerWorkflowService : IInstallerWorkflowService
{
    ...
    public async Task<FinishViewModel> GetCompletionViewModelAsync(CancellationToken cancellationToken)
    {
        ...
        return new FinishViewModel
        {
            ...
            Message = isCompleted
                ? "Zentra installation completed and locked against reinstallation."
    ...
    private static string GetSuggestedConnectionString(DatabaseProviderType? provider, bool isRunningInContainer)
    {
        ...
        {
            DatabaseProviderType.SqlServer => "Server=localhost;Database=zentra_identity;User Id=sa;Password=<strong-password>;Encrypt=True;TrustServerCertificate=False;",
            DatabaseProviderType.MySql => "Server=localhost;Port=3306;Database=zentra_identity;Uid=root;Pwd=<strong-password>;SslMode=Preferred;",
            DatabaseProviderType.PostgreSql when isRunningInContainer => "Host=postgres;Port=5432;Database=zentra;Username=zentra;Password=zentra;",
            DatabaseProviderType.PostgreSql => "Host=localhost;Port=55433;Database=zentra;Username=zentra;Password=zentra;",
            DatabaseProviderType.Sqlite => "Data Source=.data/zentra_identity.db;Mode=ReadWriteCreate;Cache=Shared;",
            _ => string.Empty
        };
    }
}
