...
using ZentraInstallerMVC.Infrastructure.Services.DatabaseProvisioning;
...
public sealed class DatabaseMigrationService : IDatabaseMigrationService
{
        ...
        CancellationToken cancellationToken)
    {
        var statements = provider switch
        {
            ...
            {
                "IF COL_LENGTH('Zentra_SecurityTokens', 'ConsumedAt') IS NULL ALTER TABLE [Zentra_SecurityTokens] ADD [ConsumedAt] datetime2 NULL;",
                "IF COL_LENGTH('Zentra_SecurityTokens', 'TokenReuseDetected') IS NULL ALTER TABLE [Zentra_SecurityTokens] ADD [TokenReuseDetected] bit NOT NULL CONSTRAINT [DF_Zentra_SecurityTokens_TokenReuseDetected] DEFAULT 0;"
            },
            DatabaseProviderType.MySql => new[]
            {
                "ALTER TABLE `Zentra_SecurityTokens` ADD COLUMN IF NOT EXISTS `ConsumedAt` datetime(6) NULL;",
                "ALTER TABLE `Zentra_SecurityTokens` ADD COLUMN IF NOT EXISTS `TokenReuseDetected` tinyint(1) NOT NULL DEFAULT FALSE;"
            },
            DatabaseProviderType.PostgreSql => new[]
            {
                "ALTER TABLE \"Zentra_SecurityTokens\" ADD COLUMN IF NOT EXISTS \"ConsumedAt\" timestamp without time zone NULL;",
                "ALTER TABLE \"Zentra_SecurityTokens\" ADD COLUMN IF NOT EXISTS \"TokenReuseDetected\" boolean NOT NULL DEFAULT FALSE;"
            },
        ...
        CancellationToken cancellationToken)
    {
        var statements = provider switch
        {
            DatabaseProviderType.SqlServer => new[]
            {
                "IF COL_LENGTH('Zentra_Clients', 'PreferredAudience') IS NULL ALTER TABLE [Zentra_Clients] ADD [PreferredAudience] nvarchar(300) NULL;"
            },
            DatabaseProviderType.MySql => new[]
            {
        ...
        CancellationToken cancellationToken)
    {
        var statements = provider switch
        {
            ...
            {
                """
                IF OBJECT_ID(N'[Zentra_ExternalIdentities]', N'U') IS NULL
                BEGIN
                    CREATE TABLE [Zentra_ExternalIdentities] (
                        ...
                        CONSTRAINT [PK_Zentra_ExternalIdentities] PRIMARY KEY ([Id]),
        ...
        CancellationToken cancellationToken)
    {
        var statements = provider switch
        {
            ...
            {
                """
                IF OBJECT_ID(N'[Zentra_NotificationProviderConfig]', N'U') IS NULL
                BEGIN
                    CREATE TABLE [Zentra_NotificationProviderConfig] (
                        [Id] uniqueidentifier NOT NULL,
                        [IsDeleted] bit NOT NULL CONSTRAINT [DF_Zentra_NPC_IsDeleted] DEFAULT (0),
                        [CreatedOn] datetime2 NOT NULL,
                        [ModifiedOn] datetime2 NULL,
                        [CreatedBy] nvarchar(255) NOT NULL,
                        [ModifiedBy] nvarchar(255) NULL,
                        ...
                        CONSTRAINT [PK_Zentra_NotificationProviderConfig] PRIMARY KEY ([Id])
                    );
                END;
                """,
                """
                IF NOT EXISTS (SELECT 1 FROM sys.indexes WHERE name = 'IX_NPC_CHANNEL_TYPE' AND object_id = OBJECT_ID('Zentra_NotificationProviderConfig'))
                    CREATE INDEX [IX_NPC_CHANNEL_TYPE] ON [Zentra_NotificationProviderConfig] ([ChannelType]);
                """,
                """
                IF NOT EXISTS (SELECT 1 FROM sys.indexes WHERE name = 'IX_NPC_CHANNEL_ACTIVE' AND object_id = OBJECT_ID('Zentra_NotificationProviderConfig'))
                    CREATE INDEX [IX_NPC_CHANNEL_ACTIVE] ON [Zentra_NotificationProviderConfig] ([ChannelType], [IsActive]);
                """
            },
            DatabaseProviderType.MySql => new[]
            {
                """
                CREATE TABLE IF NOT EXISTS `Zentra_NotificationProviderConfig` (
                    `Id` char(36) NOT NULL,
                    `IsDeleted` tinyint(1) NOT NULL DEFAULT 0,
                    `CreatedOn` datetime(6) NOT NULL,
                    `ModifiedOn` datetime(6) NULL,
            ...
            DatabaseProviderType.PostgreSql => new[]
            {
                ...
                CREATE INDEX IF NOT EXISTS "IX_NPC_CHANNEL_TYPE"
                    ON "Zentra_NotificationProviderConfig" ("ChannelType");
                """,
                """
                CREATE INDEX IF NOT EXISTS "IX_NPC_CHANNEL_ACTIVE"
                    ON "Zentra_NotificationProviderConfig" ("ChannelType", "IsActive");
                """
            },
            DatabaseProviderType.Sqlite => new[]
            {
                """
                CREATE TABLE IF NOT EXISTS "Zentra_NotificationProviderConfig" (
                    "Id" TEXT NOT NULL CONSTRAINT "PK_Zentra_NotificationProviderConfig" PRIMARY KEY,
                    "IsDeleted" INTEGER NOT NULL DEFAULT 0,
                    "CreatedOn" TEXT NOT NULL,
                    "ModifiedOn" TEXT NULL,
                    "CreatedBy" TEXT NOT NULL,
        ...
        CancellationToken cancellationToken)
    {
        var statements = provider switch
        {
            ...
            {
                """
                IF OBJECT_ID(N'[Zentra_ExternalAuthProviderConfig]', N'U') IS NULL
                BEGIN
                    CREATE TABLE [Zentra_ExternalAuthProviderConfig] (
                        [Id] uniqueidentifier NOT NULL,
                        [IsDeleted] bit NOT NULL CONSTRAINT [DF_Zentra_EAPC_IsDeleted] DEFAULT (0),
                        [CreatedOn] datetime2 NOT NULL,
                        [ModifiedOn] datetime2 NULL,
                        [CreatedBy] nvarchar(255) NOT NULL,
                        [ModifiedBy] nvarchar(255) NULL,
                        ...
                        [ConfigJson] nvarchar(max) NOT NULL,
                        [AutoProvisionEnabled] bit NOT NULL CONSTRAINT [DF_Zentra_EAPC_AutoProvision] DEFAULT (0),
                        [AllowedDomains] nvarchar(2000) NULL,
                        [LastTestedOn] datetime2 NULL,
                        [LastTestSuccess] bit NULL,
                        CONSTRAINT [PK_Zentra_ExternalAuthProviderConfig] PRIMARY KEY ([Id])
                    );
                END;
                """,
                """
                IF NOT EXISTS (SELECT 1 FROM sys.indexes WHERE name = 'IX_EAPC_PROVIDER' AND object_id = OBJECT_ID('Zentra_ExternalAuthProviderConfig'))
                    CREATE UNIQUE INDEX [IX_EAPC_PROVIDER] ON [Zentra_ExternalAuthProviderConfig] ([ProviderName]);
                """,
                """
                IF NOT EXISTS (SELECT 1 FROM sys.indexes WHERE name = 'IX_EAPC_PROVIDER_ENABLED' AND object_id = OBJECT_ID('Zentra_ExternalAuthProviderConfig'))
                    CREATE INDEX [IX_EAPC_PROVIDER_ENABLED] ON [Zentra_ExternalAuthProviderConfig] ([ProviderName], [IsEnabled]);
                """
            },
            DatabaseProviderType.MySql => new[]
            {
                """
                CREATE TABLE IF NOT EXISTS `Zentra_ExternalAuthProviderConfig` (
                    `Id` char(36) NOT NULL,
                    `IsDeleted` tinyint(1) NOT NULL DEFAULT 0,
                    `CreatedOn` datetime(6) NOT NULL,
                    `ModifiedOn` datetime(6) NULL,
            ...
            DatabaseProviderType.PostgreSql => new[]
            {
                ...
                CREATE UNIQUE INDEX IF NOT EXISTS "IX_EAPC_PROVIDER"
                    ON "Zentra_ExternalAuthProviderConfig" ("ProviderName");
                """,
                """
                CREATE INDEX IF NOT EXISTS "IX_EAPC_PROVIDER_ENABLED"
                    ON "Zentra_ExternalAuthProviderConfig" ("ProviderName", "IsEnabled");
                """
            },
            DatabaseProviderType.Sqlite => new[]
            {
                """
                CREATE TABLE IF NOT EXISTS "Zentra_ExternalAuthProviderConfig" (
                    "Id" TEXT NOT NULL CONSTRAINT "PK_Zentra_ExternalAuthProviderConfig" PRIMARY KEY,
                    "IsDeleted" INTEGER NOT NULL DEFAULT 0,
                    "CreatedOn" TEXT NOT NULL,
                    "ModifiedOn" TEXT NULL,
                    "CreatedBy" TEXT NOT NULL,
